...
png(filename=opt$out, units="in", width=10, height=6, pointsize=10, res=300)

ggplot() + 
  geom_boxplot(data = stack(my_data[1:(row_num-1),]), aes(x = ind, y = values), outlier.size=0.5) + 
  ...
  labs(x = "KEGG category", y = 'Proportion') # label of x and y axis

...
# Only need the *-Pathways.tbl files
...
def gapseq(args):
        ...
        for each_line in open(each_file):
            if not each_line.startswith('#'):
                if not each_line.startswith('ID\tName'):
                    ...
                    if pwy_pa in ['TRUE', 'true']:
                        all_detected_pwy_set.add(pwy_id)
                        current_detected_pwy_set.add(pwy_id)
        gapseq_op_dod[gnm_id] = current_detected_pwy_set

    pwy_id_list_sorted = sorted(list(all_detected_pwy_set))
    ...
    for each_gnm in sorted(list(gnm_id_set)):
        encoded_pwys = gapseq_op_dod[each_gnm]
        value_list = [each_gnm]

...
# file in
csv_file =      '/Users/songweizhi/Desktop/enrichment_analysis/COG_enrichment_analysis.csv'
output_test =   '/Users/songweizhi/Desktop/enrichment_analysis/COG_enrichment_test_results_Mann_Whitney_U.tab'
...
cog_to_group_detected_pct_dict = {}
p_value_list = []
COG_id_list = []
n = 0
for column in column_list:
    ...
    if n >= 2:
        ...
        cog_to_group_detected_pct_dict[column] = [group1_no_zero_pct, group2_no_zero_pct]
...
output_test_handle.write('COG\tKelp\tDectected_pct\tPlanktonic\tDectected_pct\tP_value\tP_value_adjusted\n')
while x < len(COG_id_list):
    ...
    current_p_detected_pct  = [str(i) for i in cog_to_group_detected_pct_dict[COG_id_list[x]]]

    output_test_handle.write('%s\t%s\t%s\t%s\t%s\t%s\t%s\n' % (COG_id_list[x], current_p_group_mean[0], current_p_detected_pct[0], current_p_group_mean[1], current_p_detected_pct[1], current_p, current_p_adjusted))

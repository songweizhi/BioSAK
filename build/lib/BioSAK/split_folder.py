...
def split_folder(args):
    ...
    while n <= folder_num:
        ...
        mkdir_cmd = 'mkdir %s' % pwd_current_folder
        os.system(mkdir_cmd)

        # copy files to new folder
        for each_file in files_in_current_folder:
            pwd_file = '%s/%s' % (file_folder, each_file)
            pwd_file_sep = '%s/%s' % (file_folder_sep, each_file)
            cp_file_cmd = 'cp %s %s/' % (pwd_file, pwd_current_folder)
